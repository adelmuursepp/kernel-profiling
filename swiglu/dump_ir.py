...
from helion_common import VARIANTS, config_key
...
def make_sample_args(tokens, d_model, hidden_dim, dtype):
    return (
        torch.randn(tokens,     d_model,    device="cuda", dtype=dtype),
        torch.randn(hidden_dim, d_model,    device="cuda", dtype=dtype),
        torch.randn(hidden_dim, d_model,    device="cuda", dtype=dtype),
    )
...
if __name__ == "__main__":
    for variant_name, kernel_fn in VARIANTS:
        for tokens, d_model, hidden_dim, dtype in MATMUL_CONFIGS:
            key = config_key(tokens, d_model, hidden_dim, dtype, variant=variant_name)
            cache_path = os.path.join(CACHE_DIR, f"{key}.json")
            if not os.path.exists(cache_path):
                print(f"skip {key} (no autotune cache)")
                continue
            print(f"\n{key}")
            dump_ir(kernel_fn, make_sample_args(tokens, d_model, hidden_dim, dtype),
