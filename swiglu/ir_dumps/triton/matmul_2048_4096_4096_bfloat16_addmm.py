...
def _helion_swiglu_kernel_fn_addmm(x, w1, w2, out):
    ...
    for offset_2 in tl.range(0, 4096, _BLOCK_SIZE_2, num_stages=3, disallow_acc_multi_buffer=True):
...
def swiglu_kernel_fn_addmm(x: torch.Tensor, w1: torch.Tensor, w2: torch.Tensor, *, _launcher=_default_launcher):
    ...
    _launcher(_helion_swiglu_kernel_fn_addmm, (triton.cdiv(2048, _BLOCK_SIZE_0) * triton.cdiv(4096, _BLOCK_SIZE_1),), x, w1, w2, out, num_warps=8, num_stages=5)
