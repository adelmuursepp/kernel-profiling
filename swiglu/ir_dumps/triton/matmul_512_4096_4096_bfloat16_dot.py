...
def _helion_swiglu_kernel_fn_dot(x, w1, w2, out):
    ...
    x_desc = tl.make_tensor_descriptor(x, [512, 4096], [4096, 1], [_BLOCK_SIZE_0, _BLOCK_SIZE_2])
    # src[helion_common.py:18]: gate_acc = hl.dot(x_tile, w1[tile_j, tile_k].T, acc=gate_acc)
    w1_desc = tl.make_tensor_descriptor(w1, [4096, 4096], [4096, 1], [_BLOCK_SIZE_1, _BLOCK_SIZE_2])
    ...
    for offset_2 in tl.range(0, 4096, _BLOCK_SIZE_2, loop_unroll_factor=1, num_stages=3, disallow_acc_multi_buffer=True, flatten=True):
        ...
        # src[helion_common.py:18]: gate_acc = hl.dot(x_tile, w1[tile_j, tile_k].T, acc=gate_acc)
        load_1 = w1_desc.load([offset_1, offset_2])
...
def swiglu_kernel_fn_dot(x: torch.Tensor, w1: torch.Tensor, w2: torch.Tensor, *, _launcher=_default_launcher):
    ...
    _launcher(_helion_swiglu_kernel_fn_dot, (triton.cdiv(4096, _BLOCK_SIZE_1) * triton.cdiv(512, _BLOCK_SIZE_0),), x, w1, w2, out, num_warps=8, num_stages=4)
