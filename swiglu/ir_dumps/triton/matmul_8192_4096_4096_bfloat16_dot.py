...
def _helion_swiglu_kernel_fn_dot(x, w1, w2, out):
    ...
    x_desc = tl.make_tensor_descriptor(x, [8192, 4096], [4096, 1], [_BLOCK_SIZE_0, _BLOCK_SIZE_2])
    # src[helion_common.py:21]: out[tile_i, tile_j] = (silu_gate * up_acc).to(x.dtype)
    out_desc = tl.make_tensor_descriptor(out, [8192, 4096], [4096, 1], [_BLOCK_SIZE_0, _BLOCK_SIZE_1])
    # src[helion_common.py:13]: for tile_i, tile_j in hl.tile([tokens, hidden_dim]):
    num_blocks_0 = tl.cdiv(4096, _BLOCK_SIZE_1)
    ...
    for offset_2 in tl.range(0, 4096, _BLOCK_SIZE_2, loop_unroll_factor=1, num_stages=4, disallow_acc_multi_buffer=False, flatten=False):
        ...
        # src[helion_common.py:18]: gate_acc = hl.dot(x_tile, w1[tile_j, tile_k].T, acc=gate_acc)
        load_1 = tl.load(w1 + (indices_1[:, None] * 4096 + indices_2[None, :] * 1), None, eviction_policy='evict_first')
        ...
        load_2 = tl.load(w2 + (indices_1[:, None] * 4096 + indices_2[None, :] * 1), None)
...
def swiglu_kernel_fn_dot(x: torch.Tensor, w1: torch.Tensor, w2: torch.Tensor, *, _launcher=_default_launcher):
    ...
    _launcher(_helion_swiglu_kernel_fn_dot, (triton.cdiv(4096, _BLOCK_SIZE_1) * triton.cdiv(8192, _BLOCK_SIZE_0),), x, w1, w2, out, num_warps=4, num_stages=4)
